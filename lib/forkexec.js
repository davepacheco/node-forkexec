...
exports.forkExecWaitShell = forkExecWaitShell;
...
/*
 * Like forkExecWait(), but where the command is specified as a string to be
 * passed to a shell.
 */
function forkExecWaitShell(args, callback)
{
	var subargs, k;

	mod_assertplus.object(args, 'args');
	mod_assertplus.string(args.command, 'args.command');
	...
	subargs.argv = [ '/bin/sh', '-c', args.command ];
	return (forkExecWait(subargs, callback));
}
